...
import com.build4all.entities.Booking;                 // booking header
import com.build4all.entities.Currency;               // currency entity
import com.build4all.entities.Item;                   // item entity
import com.build4all.entities.ItemBooking;            // item booking line
import com.build4all.entities.Users;                  // user entity
import com.build4all.repositories.BookingRepository;  // repo: booking header
import com.build4all.repositories.CurrencyRepository; // repo: currency
import com.build4all.repositories.ItemBookingsRepository; // repo: item bookings
import com.build4all.repositories.ItemRepository;     // repo: items
import com.build4all.repositories.UsersRepository;    // repo: users
import com.build4all.services.ItemBookingService;     // service interface
...
public class ItemBookingServiceImpl implements ItemBookingService {
    ...
    @Override
    public ItemBooking createBookItem(Long userId, Long itemId, int participants,
        ...
        if (capacity != null) {
            ...
            if (participants > remaining) throw new IllegalStateException("Not enough seats available");    // block
        }

        BigDecimal unit  = item.getPrice() == null ? BigDecimal.ZERO : item.getPrice();   // unit price
        BigDecimal total = unit.multiply(BigDecimal.valueOf(participants));                // total
        ...
        line.setQuantity(participants);                                                    // qty
